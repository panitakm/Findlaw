...
window.onload = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (isLawyerSaved) {
                        ...
                        saveText.innerText = 'บันทึกทนายความ';
    ...
    try {
        ...
        const feeRateText = p.fee_rate ? `${p.fee_rate} บาท` : 'ไม่ระบุ';
        ...
        if (data.schedules && data.schedules.length > 0) {
            scheduleContainer.innerHTML = data.schedules.map(s => {
                ...
                if (s.is_open) {
                    ...
                    return `<div class="schedule-row"><span class="day-name">วัน${dayName}</span><span class="schedule-badge-open">${start} - ${end}</span></div>`;
                } else {
                    return `<div class="schedule-row"><span class="day-name">วัน${dayName}</span><span class="schedule-badge-closed">ปิดทำการ</span></div>`;
                }
            }).join('');
        }
            ...
            return `
                <div class="timeline-item">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div class="timeline-title">${w.job_position}</div>
                        <span class="timeline-date">${w.year_start} - ${displayYearEnd}</span>
                    </div>
            ...
            return `
                <div class="timeline-item">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <span class="timeline-date">${e.year_start} - ${e.year_end || 'ปัจจุบัน'}</span>
                    </div>
                            ...
                            <div class="review-reply">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fa-solid fa-reply fa-rotate-180 me-2" style="font-size: 1.1rem; color: #4987A4;"></i>
                                        <span class="fw-bold" style="color: #1A435A;">${lawyerNameStr}</span>
                                    </div>
                                    <div class="d-flex align-items-center gap-3">
                                        <small class="text-muted" style="font-size: 0.82rem;">${repliedAt}</small>
                                ...
                                <div style="padding-left: 1.8rem;">
                                    <p class="mb-1" style="font-size: 0.93rem; color: #495057;">${review.reply}</p>
                                </div>
                            </div>
                        `;
                    }
                        ...
                        <div class="col-12">
                            <div class="review-card p-4 h-100">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div class="d-flex align-items-center gap-3">
                                        <img src="${avatarSrc}" class="reviewer-avatar" width="48" height="48" alt="User">
                                        <div>
                                            <div class="fw-bold mb-0" style="color: #1A435A;">${displayName}</div>
                                            <small class="text-muted">${dateFormatted}</small>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center gap-3">
                                        <div class="fs-6">${starsHtml}</div>
                                ...
                                <p class="mb-0" style="color: #495057;">${review.comment || ''}</p>
                                ${replyHtml}
                            </div>
                        </div>
                    `;
